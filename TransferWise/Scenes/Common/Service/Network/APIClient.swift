...
extension APIClient: APIClientType {
  func postRequest(_ completionHandler: @escaping (Result<[Post]>) -> Void) {
    performRequest(APIEndPoint.posts, completionHandler: completionHandler)
  }
  
  func commentsRequestFor(postId: Int, completionHandler: @escaping (Result<[Comment]>) -> Void) -> Void {
    performRequest(APIEndPoint.comments(postId: postId), completionHandler: completionHandler)
  }
}
